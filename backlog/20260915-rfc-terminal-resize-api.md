...
`take_resize()` becomes the only operation that drains resize notifications
and re-queries the terminal.
...
- `take_resize(&mut self) -> io::Result<()>` — handle a resize notification.
...
Note that `take_resize` returns no size and does not report whether the size
...
impl TerminalDriver {
    ...
    /// by [`Self::take_resize`], so a caller that never watches the signal
    ...
    /// resize notification.
    pub fn take_resize(&mut self) -> io::Result<()>;
...
- `take_resize` returns `()` rather than `bool`. A `bool` would report whether
  ...
  common path use `?`.
- A missing notification is not an error, so `take_resize` does not return
...
- `resize_signal_fd` is the existing `signal_fd` under a name that says what
  the descriptor carries. The name was `signal_fd`; the new name also matches
  `take_resize`, so the pair reads consistently ("watch `resize_signal_fd`,
  call `take_resize`").
...
- **Why prefer this over keeping `bool` return on `take_resize`?** Because
...
- **Why not return `Size` or `Option<Size>` from `take_resize`?** The size is
...
- **Why not `WouldBlock` when nothing is pending?** It is a normal outcome,
  ...
  interest registered and call `take_resize` whenever it fires. Normalizing
...
- **Alternative name: `take_resize_notification`.** Clearer but longer, and
  the `resize` in `take_resize` pairs with `resize_signal_fd`.
...
- **Alternative: keep `signal_fd` and add `handle_resize_signal`.** Rejected in
  favor of renaming the fd accessor to `resize_signal_fd`, so the fd and the
  handler share a word (`resize_signal`) and the pair reads consistently.
...
- None. The method set (`size`, `take_resize`, `resize_signal_fd`) and the
  `()` return for `take_resize` are settled.

## Future possibilities

- If tuinix ever grows an event-loop helper, this pair is the primitive such a
  loop would call: watch `resize_signal_fd`, then call `take_resize`.
